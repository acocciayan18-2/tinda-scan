...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean updateProduct(Product product) {
        ...
        values.put("cost_price", product.getCostPrice());
        values.put("stock_quantity", product.getStockQuantity());
        values.put("low_stock_alert", product.getLowStockAlert());
    ...
    public double getTodaySalesTotal() {
        ...
        // SQLite 'start of day' logic using localtime
        String query = "SELECT SUM(" + COLUMN_TRX_TOTAL_AMOUNT + ") FROM " + TABLE_TRANSACTIONS +
                " WHERE date(" + COLUMN_TRX_TIMESTAMP + ", 'localtime') = date('now', 'localtime')";
    ...
    /**
     * 2. Get Today's Gross Profit
     * Formula: SUM( (SellingPrice - CostPrice) * QuantitySold )
     */
    public double getTodayProfit() {
        double totalProfit = 0.0;
        SQLiteDatabase db = this.getReadableDatabase();

        // Join Transaction Details with Products to get Cost Price
        // Filter by Transaction Timestamp = Today
        ...
        Cursor cursor = db.rawQuery(query, null);
        if (cursor.moveToFirst()) {
            totalProfit = cursor.getDouble(0);
        }
        cursor.close();
        return totalProfit;
    }
    ...
    /**
     * 3. Get Total Unique Products (Count of rows, not sum of stock)
     */
     ...
     * Count products where stock_quantity <= low_stock_alert
     */
    public int getLowStockCount() {
        int count = 0;
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT COUNT(*) FROM " + TABLE_PRODUCTS +
                " WHERE " + COLUMN_STOCK_QTY + " <= " + COLUMN_LOW_STOCK_ALERT, null);
        if (cursor.moveToFirst()) {
            count = cursor.getInt(0);
        }
        cursor.close();
