...
public class AddProductFragment extends Fragment {
    ...
    private void setupDatePicker() {
        ...
        DatePickerDialog.OnDateSetListener dateSetListener = (picker, year, month, day) -> {
            ...
            etExpirationDate.setText(displayFormat.format(calendar.getTime()));

            // Saved internally
            SimpleDateFormat saveFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.US);
            etExpirationDate.setTag(saveFormat.format(calendar.getTime())); // <-- stored here but hidden
        };
    ...
    private void handleAddProduct() {
        String nameInput = etProductName.getText().toString().trim();
        ...
        String expirationDate = (String) etExpirationDate.getTag();

        String finalProductName = nameInput;
        ...
        // 2. 🔥 UPDATE: Add 'costPriceStr.isEmpty()' to this check
        if (finalProductName.isEmpty() || category.isEmpty() || sellingPriceStr.isEmpty() || costPriceStr.isEmpty() || stockQuantityStr.isEmpty()) {
        ...
        try {
            ...
            // 3. 🔥 UPDATE: Remove the "? 0.0" logic since it is now required
